...
# prepare commands only necessary if we are running inside cluster's node
kubectl get nodes | grep `hostname` > /dev/null && (
...
(
    ...
    get_command 180 alameda/log "kubectl get crd | egrep '^alameda' \
                                 | while read crd junk; do \
                                     echo \"kubectl get crd \${crd} -o yaml > ${workdir}/alameda/crd/crd-\${crd}.yaml\"; \
                                     echo \"kubectl get \${crd} --all-namespaces -o yaml > ${workdir}/alameda/crd/\${crd}.yaml\"; \
                                   done | sh -x"
    ## webhook
    get_command 180 alameda/log "kubectl get mutatingwebhookconfiguration | egrep '^alameda-|admission-controller' \
    ...
    get_command 180 alameda/log "kubectl get validatingwebhookconfiguration | egrep '^alameda-notifier-' \
                                 | while read wh junk; do \
    ...
    kubectl get pods --all-namespaces | egrep "${egrep_all_related_pods}" | egrep -v ' federatorai-operator-' \
        | while read ns pod junk; do
            echo "Getting xray.sh result of pod ${ns}/${pod} ..."
            dest_dir=${workdir}/${ns}.${pod}
            mkdir -pv ${dest_dir}
            # copy xray.sh from containter and execute it
            get_command 180 ${dest_dir}/log "kubectl -n ${ns} cp ${pod}:/xray.sh ${dest_dir}/xray.sh"
